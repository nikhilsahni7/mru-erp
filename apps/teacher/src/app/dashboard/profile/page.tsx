...
export default function ProfilePage() {
  ...
  const getUniqueDevices = () => {
    if (!user?.devices || !Array.isArray(user.devices) || user.devices.length === 0) return [];
      ...
      <div className="grid gap-6 md:grid-cols-[1fr_2fr]">
        ...
        <Card className="h-fit">
          <CardHeader className="text-center">
            ...
            <CardTitle className="mt-4">{user?.name || "Teacher Name"}</CardTitle>
            <CardDescription>{user?.rollNo || "Teacher ID"}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{user?.email || "teacher@example.com"}</p>
                </div>
              </div>
            ...
            <div className="space-y-6">
              {uniqueDevices.length > 0 ? (
                uniqueDevices.map((device, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-start gap-4 rounded-lg border p-4 hover:shadow-md transition-shadow">
