...
export class AttendanceController {
  ...
  private static async validateComponentSchedule(
    componentId: string,
    date: Date
  ): Promise<boolean> {
    const dayOfWeek = [
  ...
  static async createAttendanceSession(req: Request, res: Response) {
    try {
      ...
      const isAuthorized = await AttendanceService.verifyTeacherForComponent(
        teacherId,
        componentId
      );
      if (!isAuthorized) {
        return res.status(403).json({
          error: "Forbidden",
          message:
            "You are not authorized to create attendance for this course component",
        });
      }

      // Validate component schedule
      const isValidSchedule = await this.validateComponentSchedule(
        componentId,
        new Date(date)
      );
      if (!isValidSchedule) {
        return res.status(400).json({
          ...
          message: `This component doesn't have a scheduled class on ${new Date(
            date
          ).toDateString()}`,
        });
      }
  ...
  static async markAttendance(req: Request, res: Response) {
    try {
      ...
      const isAuthorized = await AttendanceService.verifyTeacherForSession(
        teacherId,
        sessionId
      );
      if (!isAuthorized) {
  ...
  static async getStudentsForAttendance(req: Request, res: Response) {
    try {
      ...
      const students = await AttendanceService.getStudentsForComponent(
        componentId
      );
  ...
  static async getAttendanceSessionDetails(req: Request, res: Response) {
    try {
      ...
      const sessionDetails =
        await AttendanceService.getAttendanceSessionDetails(sessionId);
  ...
  static async getComponentAttendanceSummary(req: Request, res: Response) {
    try {
      ...
      const summary = await AttendanceService.getComponentAttendanceSummary(
        componentId
      );
  ...
  static async getStudentCourseAttendance(req: Request, res: Response) {
    try {
      ...
      const isAuthorized = await AttendanceService.verifyTeacherForCourse(
        teacherId,
        courseId,
        currentTerm.id
      );
      if (!isAuthorized) {
        return res.status(403).json({
          error: "Forbidden",
          message:
            "You are not authorized to access this student's course attendance",
        });
      }

      const attendance = await AttendanceService.getStudentCourseAttendance(
        studentId,
        courseId
      );
  ...
  static async updateAttendanceRecord(req: Request, res: Response) {
    try {
      ...
      const isAuthorized = await AttendanceService.verifyTeacherForRecord(
        teacherId,
        recordId
      );
      if (!isAuthorized) {
    ...
    try {
      ...
      if (!isAuthorized) {
        return res.status(403).json({
          error: "Forbidden",
          message:
            "You are not authorized to access attendance for this component",
        });
      }
  ...
  static async deleteAttendanceSession(req: Request, res: Response) {
    try {
      if (!req.user || !req.user.id) {
        return res
          .status(401)
      ...
      // Verify the teacher can delete this session
      const isAuthorized = await AttendanceService.verifyTeacherForSession(
        teacherId,
        sessionId
      );
